...
# ── Upload dir: use /tmp so Railway can always write to it ────
# Files in /tmp survive the session but reset on redeploy —
...
TEMPLATE_FOLDER = _tmpl_sub if os.path.isdir(_tmpl_sub) else BASE_DIR

app = Flask(__name__,
            template_folder=TEMPLATE_FOLDER,
            static_folder=os.path.join(BASE_DIR, "static"))
app.secret_key = os.environ.get("SECRET_KEY", "change-this-in-production")
# Don't cap content length here — we stream the upload so RAM stays low
...
def init_db():
    ...
    # Seed owner account if none exists
    try:
    ...
    # Clean up expired unused pins older than 3 days
    try:
...
def login_page():
    err_map = {
        ...
        "not_in_server":    "You must be in the Zevora Discord server to log in.",
...
def favicon():
    for path in [
        os.path.join(BASE_DIR, "static", "zevora.ico"),
        os.path.join(BASE_DIR, "static", "favicon.ico"),
        os.path.join(BASE_DIR, "zevora.ico"),
    ]:
...
def dashboard():
    ...
    if user["role"] in ("owner","admin"):
        ...
        cur.execute(
            "SELECT *,pin AS pin_code,created AS created_at FROM pins WHERE agent_id=%s ORDER BY id DESC LIMIT 50",
            (user["id"],))
    ...
    # All users (admin/owner only)
    all_users = []; expired_users = []
    if user["role"] in ("owner","admin"):
        ...
        for u2 in all_users:
            ...
            u2["subscription_expired"] = (u2.get("sub_expires") and not u2["sub_active"]
                                          and u2["role"] not in ("owner","admin"))
    ...
    # My Team
    all_leagues_team = {}; team_members = []
    if user["role"] in ("owner","admin"):
        for lg in all_leagues_list:
            cur.execute("""SELECT u.id,u.username,u.role,u.discord_id,u.sub_type,u.sub_expires,
                                  u.leagues,u.avatar,
...
def team_page():
    ...
    conn = get_db(); cur = conn.cursor()
    all_leagues_list = get_all_leagues()
    all_members = []; user_leagues = []; all_scans_count = 0
    ...
    if user["role"] in ("owner","admin"):
        ...
        for lg in all_leagues_list:
            ...
            for m in members:
                ...
                m["sub_expires"] = m.get("sub_expires","")
            cur.execute("SELECT COUNT(*) FROM scans WHERE league=%s", (lg["name"],))
        ...
        # All members (deduped) for the "All" tab
        cur.execute("""SELECT u.id,u.username,u.role,u.discord_id,u.sub_type,u.sub_expires,
                              u.leagues,u.avatar,
                              COUNT(DISTINCT p.id) as pin_count,
                              COUNT(DISTINCT s.id) as scan_count
                       FROM users u
                       LEFT JOIN pins p ON p.agent_id=u.id
                       LEFT JOIN scans s ON s.pin_used=p.pin
                       GROUP BY u.id ORDER BY u.username""")
        ...
        all_scans_count = cur.fetchone()[0]
    else:
        user_slugs = [s.strip().lower() for s in (user.get("leagues") or "").split(",") if s.strip()]
        my_leagues  = [lg for lg in all_leagues_list if lg["slug"] in user_slugs]
        for lg in my_leagues:
    ...
    return render_template("team.html",
        user=user, user_leagues=user_leagues,
        all_members=all_members, all_scans_count=all_scans_count)
...
def download_report(pin):
    ...
    for key, label in [
        ...
        ("cmd_history_raw","CMD / PowerShell History"),("memory_injection_raw","Memory Injection"),
...
def api_generate_pin():
    ...
    # If user has no league set (individual subscriber) and no input, use first available league
    if not league_name:
        # Individual users (sub_type='user') can pick any league
        if user.get("sub_type") == "user":
            cur.execute("SELECT name FROM leagues LIMIT 1")
            first = cur.fetchone()
            league_name = first[0] if first else "UFF"
        else:
            cur.close(); conn.close()
            return jsonify({"error":"League required"}),400

    # Access check — skip for individual user subscribers, they can scan for any league
    if (user["role"] not in ("owner","admin")
            and user.get("sub_type") != "user"
            and not can_access_league(user, league_name)):
        cur.close(); conn.close()
    ...
    # Keep only 3 unused pins per agent
    try:
        cur.execute("""DELETE FROM pins WHERE agent_id=%s AND used=0 AND pin!=%s
                       AND id NOT IN (
                           SELECT id FROM pins WHERE agent_id=%s AND used=0 ORDER BY id DESC LIMIT 3
                       )""",
                    (user["id"], pin, user["id"]))
        conn.commit()
    except Exception:
...
def api_submit():
    try:
        ...
        # Pull all tab sections from top-level data keys
        ...
        for field in ["cleaner_info","process_hits","cleaner_hits","eventlog_hits","jumplist_hits",
                      ...
                      "yara_hits","unsigned_count","sysmain_hits","sysmain_autofail",
                      "factory_resets","drive_info","drive_warn"]:
            if field in data: report[field] = data[field]

        report.pop("vpn_info", None)  # Never store raw IP info
        report_str = json.dumps(report, default=str)

        # Trim if too large
        if len(report_str) > 2_000_000:
...
def api_create_user():
    ...
    if league_val and league_val != "ALL":
        ...
        cur_t.execute("SELECT name FROM leagues WHERE slug=%s OR name=%s",
                      (slugify(league_val), league_val))
        row = cur_t.fetchone()
        league_name = row[0] if row else league_val
    ...
    sub_expires = None
    if months > 0:
        sub_expires = (datetime.datetime.utcnow() + datetime.timedelta(days=30*months)
    ...
    cur.execute(
        """INSERT INTO users
           (discord_id,username,password_hash,role,leagues,sub_type,sub_expires,granted_by,created)
           VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",
        (discord_id, username, pw_hash, role, league_name, sub_type,
...
# ── EXE Upload (streaming — never loads whole file into RAM) ──
...
def upload_exe():
    ...
    tmp_path = EXE_FILE + ".tmp"
    size = 0
    MAX  = 180 * 1024 * 1024  # 180 MB hard cap

    try:
        with open(tmp_path, "wb") as fp:
            while True:
                chunk = f.stream.read(512 * 1024)  # 512 KB chunks — keeps RAM flat
                if not chunk:
                    break
                size += len(chunk)
                if size > MAX:
        ...
        # Atomic replace — no partial-file window
        os.replace(tmp_path, EXE_FILE)
    ...
    try:
        ...
        cur.execute(
            ...
            (json.dumps(meta),))
        cur.execute("DELETE FROM settings WHERE key='exe_data'")  # drop legacy base64 blob
        conn.commit(); cur.close(); conn.close()
    except Exception as e:
        print(f"[upload_exe] DB meta save failed: {e}")
        # File is saved — don't fail the whole request over a meta write
...
def download_exe():
    ...
    # Legacy fallback: old DB-stored base64 builds
    try:
        conn = get_db(); cur = conn.cursor()
        cur.execute("SELECT value FROM settings WHERE key='exe_data'")
        dr = cur.fetchone(); cur.close(); conn.close()
        if dr:
...
def not_found(e):
    return """<html><body style="background:#0a0a0d;color:#ececf0;font-family:monospace;
    display:flex;align-items:center;justify-content:center;height:100vh;margin:0;font-size:1.2rem">
...
def server_error(e):
    ...
    return """<html><body style="background:#0a0a0d;color:#ececf0;font-family:monospace;
    display:flex;align-items:center;justify-content:center;height:100vh;margin:0;font-size:1.2rem">
    500 — Internal server error</body></html>""", 500
